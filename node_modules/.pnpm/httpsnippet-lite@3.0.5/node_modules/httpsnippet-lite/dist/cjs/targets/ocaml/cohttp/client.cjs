...
exports.cohttp = {
    ...
    convert: ({ fullUrl, allHeaders, postData, method }, options) => {
        ...
        if (headers.length === 1) {
            push(`let headers = Header.add (Header.init ()) "${headers[0]}" "${(0, escape_js_1.escapeForDoubleQuotes)(allHeaders[headers[0]])}" in`);
        }
        else if (headers.length > 1) {
            push('let headers = Header.add_list (Header.init ()) [');
            headers.forEach(key => {
                push(`("${key}", "${(0, escape_js_1.escapeForDoubleQuotes)(allHeaders[key])}");`, 1);
            });
            push('] in');
        }
        // Add body
